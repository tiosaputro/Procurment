...
export default {
  ...
  methods: {
    ...
    getRfs(){
        this.axios.get('/api/edit-rfs-bentu/'+this.$route.params.code,{headers: {'Authorization': 'Bearer '+this.token}}).then((response)=>{
    ...
    CreateRfs() {
        this.errors = [];
        this.axios.put('/api/update-rfs-bentu/'+this.$route.params.code, this.rfs,{headers: {'Authorization': 'Bearer '+this.token}}).then(()=>{
        this.$toast.add({
          severity: "success",
