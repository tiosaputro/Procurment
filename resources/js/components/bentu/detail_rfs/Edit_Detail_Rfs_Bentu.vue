...
export default {
  ...
  methods: {
    getDetail(){
        this.axios.get('/api/edit-detail-rfs-bentu/'+this.$route.params.rfsd_id,{headers: {'Authorization': 'Bearer '+this.token}}).then((res)=>{
            this.detail = res.data;
        });
    },
    UpdateDetail() {
        this.errors = [];
        this.axios.put('/api/update-detail-rfs-bentu/'+this.$route.params.rfsd_id, this.detail,{headers: {'Authorization': 'Bearer '+this.token}}).then(()=>{
        this.$toast.add({
          severity: "success",
